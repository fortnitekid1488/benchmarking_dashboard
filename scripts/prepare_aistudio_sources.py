...
SCHEMA_PATH = ROOT / "templates" / "aistudio_financial_schema.json"
...
MODE_CONFIG = {
    "quarterly": {
        ...
        "reporting_scope": "latest reported quarter financial benchmark for the listed companies",
    ...
    "annual": {
        ...
        "reporting_scope": "latest full-year annual financial benchmark for the listed companies",
...
def write_upload_list(batch_dir: Path, batch_manifest: dict) -> None:
    ...
    lines = [
        ...
        "# Do not upload aistudio_financial_schema.json or source_manifest.json; the prompt already includes them.",
...
def prepare_batch(
    ...
    shutil.copy2(SCHEMA_PATH, batch_dir / "aistudio_financial_schema.json")
...
def main() -> int:
    ...
    parser.add_argument("--workbook", type=Path, default=ROOT / "financial_benchmark_template.xlsx")
    ...
    (latest_dir / f"latest_aistudio_{args.mode}_package_path.txt").write_text(str(output_dir), encoding="utf-8")
    (latest_dir / f"latest_aistudio_{args.mode}_{args.provider}_package_path.txt").write_text(
        str(output_dir),
        encoding="utf-8",
    )
    print(output_dir)
