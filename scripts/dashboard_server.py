...
def command_error_message(result: dict[str, Any]) -> str:
    stderr = result.get("stderr") or ""
    stdout = result.get("stdout") or ""
    text = "\n".join(part for part in (stderr, stdout) if part).strip()
    lines = [line.strip() for line in text.splitlines() if line.strip()]
    if not lines:
        return "Команда завершилась ошибкой без сообщения."
    for line in reversed(lines):
        if "Исходный workbook не найден" in line:
            return line
        if "FileNotFoundError" in line and "financial_benchmark_template.xlsx" in line:
            return "Исходный workbook не найден. Проверьте, что `financial_benchmark_template.xlsx` лежит в корне проекта."
    return lines[-1]


class DashboardHandler(BaseHTTPRequestHandler):
    server_version = "FinancialBenchmarkDashboard/1.0"
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if parsed.path == "/api/apply":
                ...
                if not result["ok"]:
                    return self.send_json({"ok": False, "error": command_error_message(result), **result}, 500)
                output_path = None
