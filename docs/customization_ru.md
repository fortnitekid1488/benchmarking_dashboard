...
Публичный шаблон должен оставаться чистым: сохраните структуру, стили, заголовки компаний, регионы и подписи строк, но не коммитьте заполненные benchmark-значения. Итоговые заполненные книги создаются только в `outputs/`.
...
- В один батч обычно кладите до 6 компаний. Скрипт дополнительно считает примерный размер промпта и файлов, авто-сплитит слишком тяжелые батчи и учитывает file-limit выбранного провайдера из `config/ai_provider_profiles.json`.
